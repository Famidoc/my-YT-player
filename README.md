...
📥 頻道自動同步與數據分析： 整合 YouTube Data API v3，只需輸入頻道代號（如 @famidoc），即可一鍵自動抓取該頻道的所有影片，並智慧批次抓取各影片的時長、發布時間、觀看數、按讚數、留言數與最新資訊。
⏱️ 影片時長與清單總時間： 影片縮圖右下角自動標註精確時間戳記，播放清單頂端即時累計顯示總時長（如「共 12 部影片 · 總時長 2 小時 45 分鐘」）。
⏯️ 斷點記憶續播 (Resume Playback)： 自動記錄每部影片的播放進度，下次開啟或切換時無縫接續播放，並給予優雅的提示 Toast。
📑 多組命名播放清單： 支援建立與管理多組自訂主題清單（如「睡前放鬆」、「深度學習」），可隨時一鍵切換不同歌單。
🖐️ 播放清單拖曳排序： 原生支援滑鼠直接拖曳排序，並為行動端觸控提供便捷的「▲/▼」微調按鈕。
📊 數據動態更新與多維排序： 內建「一鍵刷新數據」機制。總影片庫支援按「發布日期」、「影片時長」、「觀看數」、「按讚數」或「標題」進行多維度排序。
