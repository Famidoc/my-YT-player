...
📥 頻道自動同步與數據分析： 整合 YouTube Data API v3，只需輸入頻道代號（如 @famidoc），即可一鍵自動抓取該頻道的所有影片，並智慧批次抓取各影片的觀看數、按讚數、留言數與最新資訊。
📊 數據動態更新與多維排序： 內建「一鍵刷新數據」機制，隨時掌握每部影片的最新互動數據。總影片庫支援按「觀看數最高」、「按讚數最高」、「標題」等多種方式快速排序。
